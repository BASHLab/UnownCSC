...
# ============================ STAMP Dataset Extension ============================
class STAMPDataset(Dataset):
    """
    Dedicated STAMP dataloader. 
    Returns a dictionary of isolated modalities with shape (Channels, Time).
    """
    def __init__(self, root_dir, modalities, subjects, cfg):
        self.root_dir = root_dir
        self.subjects = subjects
        self.modalities = modalities
        self.base_sr = cfg.base_sample_rate
    ...
    def resample_data(self, mod_data, orig_sr):
        expected_length = int(self.duration * self.base_sr)
        resample_factor = self.base_sr / orig_sr
    ...
    def __getitem__(self, idx):
        ...
        for modality in self.modalities:
            if modality in data:
                ...
                # Resample and format to (Channels, Time) for 1D Convolutions
                resampled = self.resample_data(mod_data, orig_sr)
                x_dict[modality] = resampled.permute(1, 0)
                
        y = data.get('label', -1)
        return x_dict, y

def get_stamp_wesad_loaders(data_root, cfg, batch_size=32):
    train_dataset = STAMPDataset(data_root, cfg.modalities, cfg.train_set, cfg)
    val_dataset = STAMPDataset(data_root, cfg.modalities, cfg.val_set, cfg)
